...
fn run_v2(starting: &mut HashMap<i32, usize>, steps: usize) -> usize {
    ...
    let mut most_recent = *starting.iter_mut().max_by_key(|(_, v)| **v).unwrap().0;

    for i in len..steps {
        most_recent = step_v2(starting, most_recent, i);
    }
    most_recent as usize
}

fn step_v2(map: &mut HashMap<i32, usize>, last_spoken: i32, curr_step: usize) -> i32 {
    let word_said = if let Some(last_time) = map.get(&last_spoken) {
        curr_step - last_time
    } else {
        0
    };
    map.insert(last_spoken as i32, curr_step);
    word_said as i32
}
...
mod tests {
    ...
    #[test]
    fn test_parse_v2() {
        parse_map("16,12,1,0,15,7,11");
    }

    #[test]
    fn test_step_v2() {
        let mut a = parse_map("0,3");
        let mut next = step_v2(&mut a, 6, 3);
        assert_eq!(0, next);
        next = step_v2(&mut a, next as i32, 4);
        assert_eq!(3, next);
